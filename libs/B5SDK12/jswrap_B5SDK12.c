 ...
 *
 * This file is based on the original JavaScript interface for Bangle.js (http://www.espruino.com/Bangle.js)
 * ----------------------------------------------------------------------------
 */

#include <jswrap_B5SDK12.h>
...
#else
  jshSetPinShouldStayWatched(BTN1_PININDEX,true);
  //jshSetPinShouldStayWatched(BTN2_PININDEX,true); //SMAB5 testing
  channel = jshPinWatch(BTN1_PININDEX, true, JSPW_NONE);
  if (channel!=EV_NONE) jshSetEventCallback(channel, btn1Handler);
  //channel = jshPinWatch(BTN2_PININDEX, true, JSPW_NONE); //SMAB5 testing
  //if (channel!=EV_NONE) jshSetEventCallback(channel, btn2Handler); //SMAB5 testing
#ifdef BTN3_PININDEX
